...
<f7-page :dir="direction">
    <f7-block class="side-menu-profile" strong>
        <f7-link v-if="!user" href="/login/" data-view="#main-view" class="panel-close" icon-if-ios="f7:login" icon-if-md="material:account" icon-size="1em"> &ensp; &ensp; {{ $t("home.sideBar.login") }}</f7-link>
    ...
    <f7-list style="margin:0">
        <f7-list-group v-if="sideBarCategories.length">
            <f7-list-item v-for="row in sideBarCategories" :key="row.id" link-view="#main-view" panel-close :link="row.link_id" :title="row.name[currentLanguageId]">
                <div slot="media" v-html="row.media_ios"></div>
            </f7-list-item>
        </f7-list-group>
        ...
        <f7-list-group>
            <f7-list-item class="external" panel-close :title="$t('home.sideBar.callus.title')" link-external link="tel:920003598">
                <div slot="media"><i class='f7-icons'>phone_round_fill</i></div>
            </f7-list-item>
            <f7-list-item link="#" class="shareapp" panel-close :title="$t('home.sideBar.shareapp.title')">
                <div slot="media"><i class='icon f7-icons'>share</i></div>
            </f7-list-item>
            <f7-list-item link="#" class="rate" panel-close :title="$t('home.sideBar.rateus.title')">
                <div slot="media"><i class='icon f7-icons'>star_fill</i></div>
            </f7-list-item>
        </f7-list-group>
        <f7-list-group>
