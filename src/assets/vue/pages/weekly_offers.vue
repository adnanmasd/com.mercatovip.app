...
<f7-page name="weekly_offers" infinite @infinite="onInfinite" no-tabbar>
    ...
    <f7-list v-if="noResult && products.length == 0" media-list id="result-list" class="result">
        <f7-block>
            <h3>{{$t('vip_deals.noResult.message')}}</h3>
        </f7-block>
    </f7-list>
    <!-- Search-through list -->
    <f7-list v-else-if="loading && products.length == 0" media-list id="vip-result-list" class="result">
        <f7-row no-gap>
    ...
    <f7-list v-else media-list id="vip-result-list" class="result">
        <f7-row no-gap>
            ...
            display_mode(type, key) {
                ...
                onInfinite: function(onFilter = false) {
                    ...
                    timeout = setTimeout(function() {
                        ...
                        axios({
                            method: "GET",
                            url: api.baseUrl + api.urls.getWeeklyLimit.replace('{limit}', limit).replace('{page}', ++page),
