<template>
  <f7-page name="home" tabs hide-toolbar-on-scroll :page-content="false">
    <f7-navbar :class="this.$theme.md ? 'color-white text-color-black': ''">
      <f7-nav-left>
        <f7-link :panel-open="sideBarPos" icon="fa fa-bars"></f7-link>
      </f7-nav-left>
    ...
    <f7-toolbar
      ...
      labels
      id="mainToolbar"
      :class="this.$theme.md ? 'color-black' : ''"
      bottom-md
    >
      <a href="/" class="tab-link" data-route-tab-id="hometab">
        <i class="icon fa fa-home"></i>
        {{$t('tabbar.home.title')}}
      </a>
      <a href="/cart/" class="tab-link" data-route-tab-id="carttab">
        <i class="icon fa fa-shopping-cart">
          <span v-if="cart.total_product_count" class="badge color-red">{{cart.total_product_count}}</span>
        </i>
        {{$t('tabbar.cart.title')}}
      </a>
      <a href="/profile/" class="tab-link" data-route-tab-id="profiletab">
        <i class="icon material-icons md-only">account_box</i>
        <i class="icon f7-icons ios-only">person</i>
        {{$t('tabbar.profile.title')}}
      </a>
      <!-- <f7-link href="/" icon="fa fa-home" data-route-tab-id="hometab">
            <span class="tabbar-label">{{$t('tabbar.home.title')}}</span>
        </f7-link>
        <f7-link href="cart/" data-route-tab-id="carttab">
        ...
        <f7-link href="profile/" icon-if-md="material:account_box" icon-if-ios="f7:person"  data-route-tab-id="profiletab">
            <span class="tabbar-label">{{$t('tabbar.profile.title')}}</span>
      </f7-link>-->
    </f7-toolbar>

    <div class="tabs tabs-routable" id="maintabs">
      <div class="page-content tab" id="hometab">
        <div class="list virtual-list media-list homeContentList no-margin">
          <ul>
            <li
              v-for="(row, index) in vlData.items"
              :key="index"
              media-item
                    ...
                    <template v-for="row2 in row['value']">
                      <template v-if="row2['_link'] == 'Banners'">
                        <f7-col :width="row2['width']">
                          <span
                            class="banner-title"
                            v-if="row2.name[currentLanguageId] && row2.show_name"
                          >{{row2.name[currentLanguageId]}}</span>
                          <img
                            @click="navigate(row2.link)"
                            :src="getCMSImage(row2.image[currentLanguageId].path)"
                            class="specialBanner"
                          >
                        </f7-col>
                      </template>
                      <template v-if="row2['_link'] == 'CarouselBanner'">
                        <f7-swiper
                          :pagination="row2.length > row2.show_per_row"
                          :params="{preloadImages: false,lazy: true,freeMode: true,slidesPerView: row2.show_per_row,slidesPerColumn: 1,spaceBetween:10}"
                          style="width:100%"
                        >
                          <f7-swiper-slide v-for="i in row2.image" class="image-slider" :key="i.id">
                            ...
                            <img
                              @click="navigate(i.value.link)"
                              :src="getCMSImage(i.value['img' + currentLanguageId].path)"
                              class="banner lazy swiper-lazy"
                            >
              ...
              <template v-else v-for="row2 in row['value']">
                <template v-if="row2['_link'] == 'Sliders'">
                  <f7-row>
                    <f7-col :width="row2.width">
                      <f7-swiper
                        v-if="row2.image.length"
                        pagination
                        :params="{preloadImages: true,spaceBetween: 10,loop:true,centeredSlides:true,autoplay:true}"
                      >
                        <f7-swiper-slide class="image-slider" v-for="i in row2.image" :key="row.id">
                          <span
                            v-if="i.value['txt' + currentLanguageId]"
                            class="slider-title"
                          >{{ i.value['txt' + currentLanguageId] }}</span>
                          <img
                            @click="navigate(i.value.link)"
                            :src="getCMSImage(i.value['img' + currentLanguageId].path)"
                            class="slider lazy swiper-lazy"
                          >
                          <div class="swiper-lazy-preloader swiper-lazy-preloader-black"></div>
                        </f7-swiper-slide>
                      </f7-swiper>
                ...
                <template v-if="row2['_link'] == 'Banners'">
                  <f7-block-title
                    v-if="row2.name_as_heading && row2.show_name"
                  >{{row2.name[currentLanguageId]}}</f7-block-title>
                  <f7-block class="home-page-block" :style="row2.full_width ? 'padding : 0' : ''">
                    <f7-row :class="row2['full_width'] ? row2['full_width'] : ''">
                      <f7-col :width="row2['width']">
                        <span
                          class="banner-title"
                          v-if="row2.name[currentLanguageId] && !row2.name_as_heading  && row2.show_name"
                        >{{row2.name[currentLanguageId]}}</span>
                        <img
                          @click="navigate(row2.link)"
                          :src="getCMSImage(row2.image[currentLanguageId].path)"
                ...
                <template v-if="row2['_link'] == 'CarouselProducts'">
                  <f7-block-title
                    v-if="row2.title[currentLanguageId]"
                  >{{row2.title[currentLanguageId]}}</f7-block-title>
                  <f7-block
                    :style="row2.full_width ? 'padding : 0' : ''"
                    class="carouselHomePage home-page-block"
                  >
                    <f7-swiper
                      :params="{preloadImages: false,slidesPerView: 2.3, spaceBetween:1,loop: false,autoplay: true,freeMode: true,lazy: {loadPrevNext: true,loadPrevNextAmount: 3}}"
                    >
                      <template v-for="id in row2.product_id">
                        <product-card :product_id="id.value"></product-card>
                      </template>
                    </f7-swiper>
                  </f7-block>
                </template>
                ...
                <template v-if="row2['_link'] == 'CarouselCategory'">
                  <template v-for="cat in row2['category_id']">
                    <f7-block-title v-if="cat.value['txt' + currentLanguageId]">
                      {{cat.value['txt' + currentLanguageId]}}
                      <span class="pull-right">
                        <f7-link class="color-blue" :href="cat.value.link">{{$t("home.seeAll")}}</f7-link>
                      </span>
                    </f7-block-title>
                    <f7-block
                      :style="row2.full_width ? 'padding : 0' : ''"
        ...
        <br>
        <template v-if="homeProducts" v-for="row in homePageProducts">
          <products-list :conf="row"></products-list>
        </template>
      </div>

      <div class="page-content tab" id="carttab">
        <f7-card v-if="!cart.products" :content="$t('cart.empty')"></f7-card>
        ...
        <f7-list v-if="cart.total_product_count > 0" media-list>
          <template v-for="row in cart.products">
            <f7-list-item
              :link="'/product?product_id='+ row.product_id"
              :id="row.key"
              :title="(row.stock ? '':'***') + row.name | andFilter"
              swipeout
              @swipeout:delete="onSwipeoutDeleted"
              :class="(row.stock ? '':'text-color-red')"
            >
              <div slot="media" v-html="getImagefromSource(row.thumb)"></div>
              <span
                slot="after"
                v-html="getAfter(row.key)"
                swipeout
                :class="row.stock ? '' : 'color-red'"
              ></span>
              <div
                slot="inner"
                v-html="getProductInfo(row.quantity,row.price,row.total,row.option)"
              ></div>
              <div slot="inner">
                <template v-for="o in row.option" group-title>
                  <i class="f7-icons text-color-green">check_round_fill</i>
                  &ensp; {{o.value}}
                  <br>
                </template>
              </div>
              <f7-swipeout-actions :side="swipeoutSide">
                <f7-swipeout-button
                  v-on:click="update(row.key,row.quantity)"
                  class="update"
                  color="green"
                >{{$t('cart.item.update')}}</f7-swipeout-button>
                <f7-swipeout-button
                  delete
                  overswipe
                  :confirm-text="$t('cart.item.delete.msg')"
                  :data-confirm-title="$t('cart.item.delete.title')"
                  data-close-on-cancel="true"
                >{{$t('cart.item.delete')}}</f7-swipeout-button>
        ...
        <f7-block>
          <h4
            v-if="cart.total_product_count > 0"
            class="text-align-center"
          >{{$t('confirm.total.title')}} : {{cart.total}}</h4>
          <f7-button
            v-if="cart.total_product_count > 0"
            :disabled="cart_error.length > 0"
            style="width:100%"
            href="/checkout/"
            big
            raised
          ...
          <f7-button
            ...
            class="tab-link"
            data-tab="#hometab"
          >{{$t('cart.continue')}}</f7-button>
        </f7-block>
      </div>
      <div class="page-content tab" id="profiletab">
        <f7-list>
          <f7-list-group>
            <f7-list-item
              smart-select
              :smart-select-params="{ openIn: 'popover', closeOnSelect: true, formColorTheme: 'black' }"
        ...
        <f7-list>
          <f7-list-group v-if="user">
            ...
            >
              <div slot="media" v-if="theme.ios">
                <i class="f7-icons">person</i>
              </div>
              <div slot="media" v-if="theme.md">
            ...
            >
              <div slot="media" v-if="theme.ios">
                <i class="text-color-red f7-icons">heart_fill</i>
              </div>
              <div slot="media" v-if="theme.md">
                <i class="text-color-red material-icons">favorite</i>
              </div>
            </f7-list-item>
            <f7-list-item v-if="user" @click="logout()" link :title="$t('home.sideBar.logout')">
              <div slot="media">
                <i v-if="this.$theme.ios" class="f7-icons">logout</i>
                <i v-if="this.$theme.md" class="material-icons">close</i>
              </div>
            </f7-list-item>
          </f7-list-group>
          <f7-list-group v-else>
            <f7-list-item link="/login/" :title="$t('home.sideBar.login')">
              <div slot="media">
                <i v-if="this.$theme.ios" class="f7-icons">login</i>
                <i v-if="this.$theme.md" class="material-icons">account_circle</i>
              </div>
            </f7-list-item>
          </f7-list-group>
        </f7-list>
        ...
        <f7-list>
          <f7-list-group v-if="sideBarTools.length">
            <f7-list-item
              ...
              panel-close
              :link="row.link_id"
              v-for="row in sideBarTools"
              :key="row.id"
              :title="row.name[currentLanguageId]"
            >
        ...
        <f7-list>
          <f7-list-group>
            ...
            <f7-list-item link="/contact" panel-close :title="$t('home.sideBar.contactus.title')">
              <div slot="media" v-if="theme.ios">
                <i class="icon f7-icons">email</i>
              </div>
              <div slot="media" v-if="theme.md">
                <i class="icon material-icons">alternate_email</i>
              </div>
            </f7-list-item>
            <f7-list-item
              link="#"
              class="shareapp"
              panel-close
              :title="$t('home.sideBar.shareapp.title')"
            >
              <div slot="media" v-if="theme.ios">
            ...
            <f7-list-item
              ...
              class="rate"
              panel-close
              :title="$t('home.sideBar.rateus.title')"
            >
              <div slot="media" v-if="theme.ios">
                <i class="icon f7-icons" style="color:orange">star_fill</i>
              </div>
              <div slot="media" v-if="theme.md">
                <i class="icon material-icons" style="color:orange">star_fill</i>
              </div>
            </f7-list-item>
          </f7-list-group>
        </f7-list>
        <f7-block
          inset
          v-html="'v' + this.$f7.version + ' ' + $t('settings.copyright.statement')"
          class="text-align-center"
        ></f7-block>
      </div>
    </div>
...
import ProductsList from "../CustomComponents/ProductsList.vue";
...
export default {
    ...
    return {
      ...
      items:
        localStorage.getItem("recentSearch") == null
          ? []
          : JSON.parse(localStorage.getItem("recentSearch").split(",")),
      ...
      sideBarPos:
        localStorage.getItem("language_id") === "1" ? "left" : "right",
      sideBarPos2:
        localStorage.getItem("language_id") === "1" ? "right" : "left",
      logo:
        localStorage.getItem("language_id") == 1
          ? "static/img/logo-en.png"
          : "static/img/logo-ar.png",
      searchTerm: "",
      ...
      recentView:
        localStorage.getItem("recentView") == null
          ? []
          : JSON.parse(localStorage.getItem("recentView").split(","))
    };
  },
  created() {
    self = this;
    ...
    axios
      .get(cms.baseUrl + cms.getReigion("home") + cms.tokenVar)
  ...
  mounted: function() {
    ...
    axios
      .get(cms.baseUrl + cms.getCollection("ProductsList") + cms.tokenVar)
      .then(function(response) {
        self.homeProducts = true;
        self.homePageProducts = response.data.entries;
      });
  },
  filters: {
    andFilter(val) {
  ...
  methods: {
    ...
    shareapp() {
      window.plugins.socialsharing.share(
        this.$t("share.app.msg"),
        this.$t("share.app.title"),
        /*this.$t('share.app.image')*/ null,
        this.$t("share.app.link")
      );
    },
    logout() {
      self.$f7.preloader.show();
    ...
    checkVersion() {
      ...
      let version;
      let appId;
      let currentVersion = this.$f7.version;
      let newVersion = "";
      ...
      self.$f7.request({
        ...
        method: "GET",
        url: cms.baseUrl + cms.getSingleton("Version") + cms.tokenVar,
        success: function(e, status, xhr) {
          version =  e;
        }
      });
      newVersion = version ? version["version_" + device.platform.toLowerCase()] : ""; 
      //newVersion = '2.1.6';
      if (
        version &&
        newVersion &&
        ...
        self.$f7.dialog.alert(
          self.$t("app.update.text"),
          self.$t("app.update.title"),
          function() {
            //LaunchReview.launch(null, appId);
            window.open( version["url_" + device.platform.toLowerCase()], '_system'); return false;
          }
        );
      }
    },
